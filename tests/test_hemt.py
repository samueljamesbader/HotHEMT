...
def test_analytical_2d3d():
    ...
    We set l_chipy=w_f=L_p to get an effectively 2D problem,
    and k_GaN=k_Rel=k_Sub for the simplified expression.
    """
    with specific_sim_work_dir("output/output_test/test_hemt_analytical_2d3d", clear=True):
        h=HEMT(
            n_f=1, **common, w_f=2*um,
            rows=1, row_pitch=7*um,
            L_h=0.2*um, t_GaN=0.5*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=2*um, l_chipy=2*um,
...
def test_hemt3d(show_gui:bool=False):
    ...
    with specific_sim_work_dir("output/output_test/test_hemt3d", clear=True):
        # Baseline case
        ...
        h=HEMT(
            ...
            rows=1, row_pitch=7*um,
            L_h=0.2*um, t_GaN=0.5*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=10*um, l_chipy=10*um, h_bot=1e9*W/(K*m**2),
        ...
        h=HEMT(
            n_f=1, **common, w_f=2*um,
            rows=1, row_pitch=7*um,
            L_h=0.2*um, t_GaN=0.1*um, t_Rel=.4*um, t_Sub=.5*um,
            l_chipx=10*um, l_chipy=10*um, h_bot=1e9*W/(K*m**2),
        ...
        # Baseline case with non-square simulation domain
        # Analytical 12.41 Kmm/W
        h=HEMT(
            n_f=1, **common, w_f=2*um,
            rows=1, row_pitch=7*um,
            L_h=0.2*um, t_GaN=0.5*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=10*um, l_chipy=3*um, h_bot=1e9*W/(K*m**2),
            k300_GaN=150*W/(m*K),k300_Rel=10*W/(m*K),k300_Sub=10*W/(m*K),
            nthr_GaN=0, nthr_Rel=0, nthr_Sub=0
        )
        h.get_mesh_path(force_remesh=True, show_gui=show_gui)
        r3dsim=h.get_siminfo(force_resim=True)['RthWEff3D Heater Avg [K.mm/W]'] * (K*mm/W)
        r3dcal=analytical_RthWEff3D(h)
        assert np.isclose(r3dcal, 12.41*K*mm/W, rtol=1e-2)
        assert np.isclose(r3dsim, r3dcal, rtol=.05), f"Simulated {r3dsim}, calculated {r3dcal}"
        ...
        h=HEMT(
            ...
            rows=1, row_pitch=7*um,
            L_h=0.2*um, t_GaN=0.5*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=6*um, l_chipy=6*um, h_bot=1e9*W/(K*m**2),
            k300_GaN=150*W/(m*K),k300_Rel=10*W/(m*K),k300_Sub=10*W/(m*K),
            nthr_GaN=0, nthr_Rel=0, nthr_Sub=0, dont_mesh_contacts=True
        )
        h.get_mesh_path(force_remesh=True, show_gui=show_gui)
        ...
        h=HEMT(
            n_f=1, **common, w_f=2*um,
            rows=1, row_pitch=7*um,
            L_h=0.2*um, t_GaN=0.1*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=3*um, l_chipy=3*um, h_bot=1e9*W/(K*m**2),
            k300_GaN=150*W/(m*K),k300_Rel=10*W/(m*K),k300_Sub=10*W/(m*K),
            nthr_GaN=0, nthr_Rel=0, nthr_Sub=0, dont_mesh_contacts=True,
        )
        h.get_mesh_path(force_remesh=True, show_gui=show_gui)
        ...
        h=HEMT(
            n_f=1, **common, w_f=2*um,
            rows=1, row_pitch=7*um,
            L_h=0.05*um, t_GaN=0.5*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=10*um, l_chipy=10*um, h_bot=1e9*W/(K*m**2),
            k300_GaN=150*W/(m*K),k300_Rel=10*W/(m*K),k300_Sub=10*W/(m*K),
            nthr_GaN=0, nthr_Rel=0, nthr_Sub=0, dont_mesh_contacts=True
        )
        ...
        # Baseline but test T-dependent thermal conductivity
        # Analytical 12.30 Kmm/W
        h=HEMT(
            n_f=1, **common, w_f=2*um,
            rows=1, row_pitch=7*um,
            L_h=0.2*um, t_GaN=.5*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=10*um, l_chipy=10*um, h_bot=1e10*W/(K*m**2),
            k300_GaN=150*W/(m*K),k300_Rel=10*W/(m*K),k300_Sub=10*W/(m*K),
        ...
        h=HEMT(
            ...
            rows=1, row_pitch=7*um,
            L_h=0.005*um, t_GaN=0.5*um, t_Rel=.4*um, t_Sub=.1*um,
            l_chipx=3*um, l_chipy=3*um, h_bot=1e9*W/(K*m**2),
            k300_GaN=150*W/(m*K),k300_Rel=10*W/(m*K),k300_Sub=10*W/(m*K),
            nthr_GaN=0, nthr_Rel=0, nthr_Sub=0, dont_mesh_contacts=True
        )
        ...
        h=HEMT(
            ...
            rows=1, row_pitch=7*um,
            L_h=3*um, t_GaN=0.5*um, t_Rel=.5*um,
            l_chipx=20*um, l_chipy=20*um,
            #h_bot=1e10*W/(K*m**2),
